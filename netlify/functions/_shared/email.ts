...
export async function sendRegistrationConfirmation(
  ...
  <div style="font-family:Inter,Arial,sans-serif;background:#070e1d;color:#F5F0E8;padding:32px;">
    <div style="max-width:560px;margin:0 auto;border:1px solid rgba(255,208,0,0.25);border-radius:8px;overflow:hidden;">
      ...
      <div style="padding:28px;">
        ...
        <p style="font-size:15px;">Dear ${data.name},</p>
        ...
        <table style="width:100%;font-size:14px;margin:18px 0;border-collapse:collapse;">
          <tr><td style="padding:6px 0;color:#8a8a8a;">Application ID</td><td style="padding:6px 0;color:#FFD000;font-weight:bold;text-align:right;">${data.applicationId}</td></tr>
          <tr><td style="padding:6px 0;color:#8a8a8a;">Registration Type</td><td style="padding:6px 0;text-align:right;">${data.registrationType}</td></tr>
          ${data.committee ? `<tr><td style="padding:6px 0;color:#8a8a8a;">Committee</td><td style="padding:6px 0;text-align:right;">${data.committee}</td></tr>` : ''}
        </table>
        <p style="font-size:14px;line-height:1.6;color:#cfcccc;">
        ...
        ${data.retrieveUrl ? `<p style="font-size:13px;color:#8a8a8a;">You can re-download your PDF anytime at: <a href="${data.retrieveUrl}" style="color:#FFD000;">${data.retrieveUrl}</a></p>` : ''}
  ...
  await transporter().sendMail({
    from: process.env.EMAIL_FROM || 'CoMUN 2026 <noreply@cottons.edu>',
    to,
    subject: `CoMUN 2026 — Registration Confirmed (${data.applicationId})`,
    html,
  });
}
